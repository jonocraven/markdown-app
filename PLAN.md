...
**Correction 5 (post-v1, on real user feedback):** Correction 4 was still wrong — `window.print()` from JS is a documented dead end specifically on macOS WKWebView (it works on Linux/Windows webviews, which is what made the earlier fix look plausible). This is exactly why Tauri ships a separate Rust-side `Webview::print()`, backed natively by wry, for macOS. Fixed for real by handling the `"print"` menu id entirely in `on_menu_event` (`lib.rs`) — it now calls `app.get_webview_window("main")` and `.print()` directly in Rust and never round-trips to the frontend at all; the dead `case "print"` in App.tsx's `onMenuEvent` switch was removed. Also fixed in this round: the dock icon's master PNG had the squircle filling ~94% of the 1024×1024 canvas with almost no margin, which read as oversized next to other Dock icons that keep Apple's usual ~80–82% safe area. Regenerated `icons/icon-source.png` at the correct ~82% content ratio (same `#` glyph, same terracotta field) and re-ran `npx tauri icon` to refresh every size/format.
