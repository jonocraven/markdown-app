 ...
 *
 * Dual-themed (vitesse-light + vitesse-dark, PLAN.md dark-mode addendum):
 * codeToHast emits both palettes as CSS custom properties on every token
 * span (--shiki-light/--shiki-dark plus the light default inline as
 * `color`), so a theme switch is a pure CSS override (reader.css) rather
 * than a re-highlight. The background is overridden to paper-3 in CSS, and
